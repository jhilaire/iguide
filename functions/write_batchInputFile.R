write_batchInputFile <- function(i_singleTree, i_treeType, i_fpath) {

  v_fname_out  <- i_fpath$out
  v_fname_desc <- i_fpath$desc
  v_fname_tex  <- i_fpath$tex  
  v_fname_fn   <- i_fpath$fn
  ...
  if (i_singleTree) {  
    ...
    catret("  24.1      (version of GUIDE that generated this file)", file=i_fpath[["in"]], append=T)
    ...
    if (substr(i_treeType,1,1) == "Q") {
      ...
      #file.copy(i_fpath[["in"]], file.path(v_outPath, v_fname_in))
    }
